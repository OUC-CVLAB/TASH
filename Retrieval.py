...
def Evaluate_mAP(device, gallery_codes, query_codes, gallery_labels, query_labels, Top_N=None, sameQueryDB=True):##sea animals sameQueryDB=False
...
def DoRetrieval(device, net, Img_dir, Gallery_dir, Query_dir, NB_CLS, Top_N, args):
    Gallery_set = ImageList(Img_dir, open(Gallery_dir).readlines())
    ...
    with T.no_grad():
        for i, data in enumerate(Gallery_loader, 0):
            ...
            if gallery_y_batch.dim() == 1:
                ...
                gallery_y_batch = gallery_y_batch.to(T.float)
        ...
        for i, data in enumerate(Query_loader, 0):
            ...
            if query_y_batch.dim() == 1:
                ...
                query_y_batch = query_y_batch.to(T.float)
